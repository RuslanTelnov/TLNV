...
# Category code from create_from_wb log (Legacy mapper output)
...
try:
    ...
    # Filter for the ones that failed
    target_codes = ["Toys*Age", "Toys*Gender", "Stuffed toys*View"]
    
    for attr in attrs:
        if attr['code'] in target_codes:
            print(f"\n--- {attr['code']} ---")
            print(f"Mandatory: {attr['mandatory']}")
